...
void throw_when_nonsymmetric(Mat mat);
void eigensort(Vec &vals, Mat &vecs);
...
struct Jacobi {
  ...
  Jacobi(Mat &mat)
      ...
      for (int p = 0; p < m_n - 1; p++) {
        for (int q = p + 1; q < m_n; q++) {
          ...
          auto mat_pq_is_tiny_compared_to =
              [target = 100.0 * abs(m_mat[p][q])](double value) -> bool {
            return target <= numeric_limits<double>::epsilon() * abs(value);
          };
          ...
          if (abs(m_mat[p][q]) > thresh) {
            ...
            if (mat_pq_is_tiny_compared_to(cot_2phi_numerator)) {
              ...
              tan_phi = 1.0 / (abs(cot_2phi) + sqrt(1.0 + cot_2phi * cot_2phi));
              if (cot_2phi < 0.0) {
                tan_phi = -tan_phi;
              }
            }
    ...
    throw(ostringstream() << "ERROR: 反復処理の繰り返し回数が許容限度（"
                          << LIMIT << "回）を超えました．")
        .str();
  }
...
void throw_when_nonsymmetric(Mat mat) {
  if (mat.m_rows != mat.m_cols) {
    throw string("ERROR: 行と列の次数が一致していません．");
  }
  int n = mat.m_rows;
  for (int r = 0; r < n; r++) {
    for (int c = r + 1; c < n; c++) {
...
void eigensort(Vec &vals, Mat &vecs) {
  ...
  for (int i = 0; i < n - 1; i++) {
    ...
    swap(vals[max_i], vals[i]);
    for (int r = 0; r < n; r++) {
      swap(vecs[r][i], vecs[r][max_i]);
    }
  }
}
